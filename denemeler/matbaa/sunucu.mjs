...
import { anasayfa, ogrenciler, sirketler } from './gorunum/sayfalar.mjs';
...
yol.get('/', (istek, cevap) => {
  ...
  gonder(cevap, anasayfa({
    kisi: istek.kisi,
    etkinlikler: S.etkinlikler.all(),
    konusmacilar: S.konusmacilar.all().slice(0, 12),
    kurumlar,
    serit,
...
const OY_S = {
  ...
  sil: db.prepare('DELETE FROM oylar WHERE surum = ? AND cerez = ?'),
...
yol.post('/api/oy', (istek, cevap) => {
  ...
  if (oy !== 1 && oy !== -1 && oy !== 0) return cevap.status(400).json({ hata: 'oy' });
  ...
  // oy = 0: kullanıcı oyunu geri alıyor
  if (oy === 0) {
    OY_S.sil.run(surum, kimlik);
    return cevap.set('Cache-Control', 'no-store').json({ tamam: true, geriAlindi: true, ozet: OY_S.ozet.all() });
  }
...
app.listen(PORT, '127.0.0.1', () =>
  console.log(`compec v2 (matbaa) hazir: http://127.0.0.1:${PORT}${TEMEL || ''}/`));
