...
<section class="bolum">
  <div class="kap">
    ...
    <div class="arsiv gorselli">
      ${etkinlikler.map((e) => html`
        <a class="satir" href="${u('/etkinlik/' + e.slug)}">
          <div class="satir-yil">${e.yil || ''}</div>
          <div class="satir-kare">
            ${e.foto
              ? html`<img src="${foto(e.foto)}" alt="${e.ad} etkinliğinden bir kare" loading="lazy">`
              : html`<span aria-hidden="true">${(e.ad || '?').trim().charAt(0).toLocaleUpperCase('tr')}</span>`}
          </div>
          <div>
...
/* ============================ ETKİNLİK AYRINTI ============================ */
export function etkinlikSayfasi({ kisi, e, baskilar, konusmacilar, sponsorlar, odulSayisi }) {
...
<section class="bolum">
  <div class="kap dar">
    ...
    <p style="margin-top:22px">
      <a class="dugme sade" href="${u('/etkinlikler')}">Tüm etkinlikler</a>
      ${e.slug === 'bilisim-odulleri' ? html`<a class="dugme" href="${u('/oduller')}" style="margin-left:10px">Kazanan arşivi${odulSayisi ? html` (${sayi(odulSayisi)} kayıt)` : ''}</a>` : ''}
    </p>
  </div>
</section>`;
  ...
  <div class="kap">
    ...
    ${yillar.map((yil) => html`
      <div style="margin-bottom:34px">
        <h3 style="font-family:var(--mono);font-size:12.5px;color:var(--metin-3);font-weight:500;padding-bottom:9px;border-bottom:1px solid var(--cizgi)">${yil}</h3>
