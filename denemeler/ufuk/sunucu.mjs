...
import { kanitSayfasi, kimlerleSayfasi, odulSayfasi, arsivSayfasi } from './gorunum/kanit.mjs';
...
/* ---------------- sorgular ---------------- */
/* Etkinlik fotografi v7'ye OZEL. etkinlikler.foto kolonu v1-v6 ile PAYLASILIYOR;
   oraya v7'nin karelerini yazmak v3/v4/v5/v6 sayfalarinda 404 uretti, cunku o
   dosyalar yalnizca ufuk/ agacinda var. Bu yuzden v7 kendi eslesmesini v7_fotolar
   tablosunda tutuyor ve asagidaki sorgular once ona, yoksa paylasilan kolona bakiyor.
   Yeni kare eklerken tohum-v7.mjs'i kullan, etkinlikler.foto'ya DOKUNMA. */
const ETK_FOTO = `COALESCE((SELECT f.foto FROM v7_fotolar f WHERE f.slug = e.slug), e.foto) AS foto`;

const S = {
  etkinlikler: db.prepare(`SELECT e.*, ${ETK_FOTO} FROM etkinlikler e
                           WHERE e.yayinda = 1 ORDER BY e.sira`),
  ...
  kanitlar: db.prepare('SELECT * FROM kanitlar ORDER BY konu, sira'),
  kunye: db.prepare(`SELECT * FROM kanitlar
                     WHERE deger IS NOT NULL AND etiket='dogrulanmis'
                       AND iddia IN ('Kuruluş','TechSummit baskı sayısı','DataCamp baskı sayısı',
                                     'TechSummit 2022 kaydı','Bilişim Ödülleri 2018 oyu, 20 gün içinde')
                     ORDER BY konu, sira`),
  geziler: db.prepare('SELECT * FROM gezileri ORDER BY sira'),
  oduller: db.prepare('SELECT * FROM oduller ORDER BY yil DESC, sira'),
  kilometre: db.prepare('SELECT * FROM kilometre ORDER BY sira'),
  sonBaskilar: db.prepare(`SELECT b.*, e.ad FROM baskilar b JOIN etkinlikler e ON e.slug = b.etkinlik
                           ORDER BY b.yil DESC, b.no DESC LIMIT 8`),
  kanitliOzet: db.prepare(`SELECT * FROM kanitlar WHERE deger IS NOT NULL AND etiket='dogrulanmis'
                           AND iddia IN ('TechSummit 2022 kaydı','Bilişim Ödülleri 2018 oyu, 20 gün içinde',
                                         'Digitalized 2022 kaydı','TechSummit 2019 katılımcısı')
                           ORDER BY sira`),
  odulSecki: db.prepare(`SELECT * FROM oduller WHERE kazanan IN
    ('Netflix','Yemeksepeti','Ekşi Sözlük','Google','Getir','Trendyol','Spotify','sahibinden.com')
    GROUP BY kazanan ORDER BY yil DESC LIMIT 8`),
  sayim: db.prepare(`SELECT
      (SELECT COUNT(*) FROM oduller) odul,
      (SELECT COUNT(DISTINCT yil) FROM oduller) odulYil,
      (SELECT COUNT(*) FROM baskilar) baski,
      (SELECT COUNT(*) FROM konusmacilar) konusmaci,
      (SELECT COUNT(*) FROM kurumlar) kurum`),
  kadro: db.prepare('SELECT * FROM konusmacilar ORDER BY yil DESC, sira'),
  etkinlikSlug: db.prepare(`SELECT e.*, ${ETK_FOTO} FROM etkinlikler e
                            WHERE e.slug = ? AND e.yayinda = 1`),
...
yol.get('/', (istek, cevap) => {
  ...
  const kadro = S.kadro.all();
  const sy = S.sayim.get();
  gonder(cevap, anasayfa({
    kisi: istek.kisi,
    gemiler: AMIRAL.map((x) => hepsi.find((e) => e.slug === x)).filter(Boolean),
    digerleri: hepsi.filter((e) => !AMIRAL.includes(e.slug)),
    serit,
    logolar: logoEslemesi([...kurumlar, ...serit, ...kadro.map((k) => ({ ad: k.kurum }))]),
    ekip: S.guncelEkip.all(),
    kadro: kadro.slice(0, 8),
    kanitli: S.kanitliOzet.all(),
    kilometreler: S.kilometre.all(),
    secki: S.odulSecki.all(),
    sayimlar: sy,
    uyeSayisi: S.uyeSayisi.get().c,
  }));
});

yol.get('/arsiv', (istek, cevap) => {
  const sy = S.sayim.get();
  gonder(cevap, arsivSayfasi({
    kisi: istek.kisi,
    sayimlar: { ...sy, toplam: sy.odul + sy.baski + sy.konusmaci + sy.kurum },
    sonBaskilar: S.sonBaskilar.all(),
  }));
});

yol.get('/kanit', (istek, cevap) => {
  const hepsi = S.kanitlar.all();
  const sira = ['olcek', 'sureklilik', 'topluluk', 'erisim', 'eksik'];
  const gruplar = sira
    .map((k) => [k, hepsi.filter((s) => s.konu === k)])
    .filter(([, s]) => s.length);
  const sayim = {};
  for (const s of hepsi) sayim[s.etiket] = (sayim[s.etiket] || 0) + 1;
  gonder(cevap, kanitSayfasi({ kisi: istek.kisi, gruplar, sayim }));
});

yol.get('/oduller', (istek, cevap) => {
  const hepsi = S.oduller.all();
  const gruplu = new Map();
  for (const o of hepsi) {
    if (!gruplu.has(o.yil)) gruplu.set(o.yil, []);
    gruplu.get(o.yil).push(o);
  }
  gonder(cevap, odulSayfasi({
    kisi: istek.kisi,
    yillar: [...gruplu.entries()].sort((a, b) => b[0] - a[0]),
    toplam: hepsi.length,
    kategoriSayisi: new Set(hepsi.map((o) => o.kategori)).size,
  }));
});

yol.get('/kimlerle', (istek, cevap) => {
  const kadro = S.kadro.all();
  const kurumlar = [...new Set(kadro.map((k) => k.kurum).filter(Boolean))];
  gonder(cevap, kimlerleSayfasi({
    kisi: istek.kisi, kadro,
    logolar: logoEslemesi(kurumlar.map((ad) => ({ ad }))),
    kurumSayisi: kurumlar.length,
  }));
});

yol.get('/ogrenciler', (istek, cevap) => {
  const kadro = S.kadro.all();
  gonder(cevap, ogrenciler({
    kisi: istek.kisi,
    konusmacilar: S.konusmacilarEtkinlik.all('datacamp', 2025),
    geziler: S.geziler.all(),
    biletler: S.kanitlar.all().filter((k) => k.konu === 'erisim' && k.deger && k.deger !== 'Ücretsiz'),
    kurumSayisi: new Set(kadro.map((k) => k.kurum).filter(Boolean)).size,
  }));
});
...
yol.get('/etkinlik/:slug', (istek, cevap) => {
  ...
  gonder(cevap, etkinlikSayfasi({
    ...
    // Ödül arşivi bağlantısındaki sayı elle yazılmasin: bir ara 250 yaziyordu,
    // gercek kayit 222'ydi. Kaynak tek yer olsun, tablonun kendisi.
    odulSayisi: S.sayim.get().odul,
  }));
});
...
app.listen(PORT, '127.0.0.1', () =>
  console.log(`compec v7 (ufuk) hazir: http://127.0.0.1:${PORT}${TEMEL || ''}/`));
