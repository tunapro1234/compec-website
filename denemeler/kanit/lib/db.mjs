...
-- Kanıt katmanı: sitedeki her rakamın kaynağı ve doğrulama durumu
CREATE TABLE IF NOT EXISTS kanitlar (
  id        INTEGER PRIMARY KEY,
  konu      TEXT NOT NULL,           -- hangi başlık altında
  iddia     TEXT NOT NULL,           -- ne diyoruz
  deger     TEXT,                    -- rakam varsa
  kaynak    TEXT,                    -- kim söylüyor (eksik kayıtlarda boş olabilir)
  kaynak_url TEXT,
  etiket    TEXT NOT NULL,           -- dogrulanmis | kulup-ici | supheli | eksik
  aciklama  TEXT,
  sira      INTEGER NOT NULL DEFAULT 100
);
CREATE INDEX IF NOT EXISTS ix_kanit_konu ON kanitlar(konu, sira);

CREATE TABLE IF NOT EXISTS gezileri (
  id     INTEGER PRIMARY KEY,
  kurum  TEXT NOT NULL,
  yil    INTEGER,
  sira   INTEGER NOT NULL DEFAULT 100
